...
namespace SiteForAdaptation.Controllers
{
    public class ManagerController : Controller
    {
        ...
        public ManagerController(ILogger<HomeController> logger, DataContext context)
        {
            _logger = logger;
            _context = context;
        }

        string userTypesTittle = "Руководитель";
        ...
        public IActionResult FullIndex(int companyId)
        {
            ...
            data = _context.Companies
                .Where(c => c.IsHide == false)
                .Include(c => c.Contact).ThenInclude(c => c.Items)
            ...
            var memoForManager = new MemoForManager();
            memoForManager = _context.MemoForManagers
                .Include(c => c.Company)
            ...
            var viewModal = new ManagerViewModel
            {
                companyId = data.Id,
                ...
                Tittle_1 = memoForManager?.Tittle_1,
                Subtittle_1 = memoForManager?.Subtittle_1,
                Tittle_2 = memoForManager?.Tittle_2,
                Subtittle_2 = memoForManager?.Subtittle_2,
                Tittle_3 = memoForManager?.Tittle_3,
                Subtittle_3 = memoForManager?.Subtittle_3,
                Tittle_4 = memoForManager?.Tittle_4,
                Subtittle_4 = memoForManager?.Subtittle_4,
                Tittle_5 = memoForManager?.Tittle_5,
                Subtittle_5 = memoForManager?.Subtittle_5,
            };
            ...
            //foreach (var item in userType.StoryMaps)
            //{
            //    var storyMap = new StoryMapViewModel
            //    {
            //        storyMapName = item.Name,
            //        storyMapNumber = item.Number,
            //        //storyImagePath = item.ImagePath,
            //        storyVideoPath = item.VideoPath,
            //        storyFilePath = item.FilePath
            //    };

            //    foreach (var iitemStoryItemstem in item.StoryItems)
            //    {
            //        var storyItem = new StoryItemViewModel
            //        {
            //            storyItemTittle = iitemStoryItemstem.Tittle
            //        };

            //        storyMap.storyItems.Add(storyItem);
            //    };

            //    viewModal.storyMaps.Add(storyMap);
            //}
            //userTaskItems
            if (null != userTask)
            {
